...
const pool = new Pool({
  ...
  password: '1964', //This should be changed to reflect the password you used when setting up Postgres
  port: 5432,
});
...
async function addTodo(task) {
  ...
  console.log(`Added todo: ${response.rows[0].task}`);
  };
...
async function completeTodo(id) {
  ...
  if (response.rowCount < 0) {
    ...
    console.log(`No item was found with id ${id}.`);
  }
...
async function deleteTodo(id) {
  ...
  const response = await pool.query(query, [id]);
  if (response.rowCount < 0) {
    console.log(`Deleted item id ${id} from the list.`);
  }
  else {
    console.log(`No item was found with id ${id}.`);
  }
...
async function clearCompleted() {
  ...
  const response = await pool.query(query);
  console.log(`Cleared ${response.rowCount} completed items from the list.`);
